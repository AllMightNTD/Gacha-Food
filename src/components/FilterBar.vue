<template>
  <div class="w-full px-4 py-2.5 space-y-2">
    <!-- Category Pills Scrollable -->
    <div class="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1 text-xs">
      <button
        v-for="cat in categories"
        :key="cat.id"
        @click="setCategory(cat.id)"
        class="whitespace-nowrap px-3 py-1.5 rounded-full font-bold transition-all duration-200 shadow-sm flex items-center gap-1 shrink-0"
        :class="selectedCategory === cat.id
          ? 'bg-rose-500 text-white shadow-rose-200 scale-105'
          : 'bg-white/80 text-gray-600 hover:bg-rose-50 border border-rose-100/60'"
      >
        <span>{{ cat.icon }}</span>
        <span>{{ cat.label }}</span>
      </button>
    </div>

    <!-- Budget Filter Pills -->
    <div class="flex items-center justify-between gap-1.5 bg-white/60 p-1 rounded-2xl border border-pink-100 text-xs">
      <button
        v-for="b in budgetOptions"
        :key="b.id"
        @click="setBudget(b.id)"
        class="flex-1 py-1 px-2 rounded-xl font-bold transition-all flex items-center justify-center gap-1"
        :class="selectedBudget === b.id
          ? 'bg-gradient-to-r from-amber-400 to-rose-400 text-white shadow-sm'
          : 'text-gray-500 hover:text-gray-800'"
      >
        <span>{{ b.icon }}</span>
        <span>{{ b.label }}</span>
      </button>
    </div>
  </div>
</template>

<script setup>
import { useFoodStore } from '../composables/useFoodStore.js'

const { selectedCategory, selectedBudget, setCategory, setBudget } = useFoodStore()

const categories = [
  { id: 'all', label: 'Tất cả món', icon: '✨' },
  { id: 'hotpot', label: 'Lẩu & Đồ nước', icon: '🍲' },
  { id: 'bbq', label: 'Nướng & Xèo', icon: '🥩' },
  { id: 'noodle', label: 'Bún & Phở', icon: '🍜' },
  { id: 'rice', label: 'Cơm chắc dạ', icon: '🍛' },
  { id: 'snack', label: 'Ăn vặt', icon: '🥟' },
  { id: 'dessert', label: 'Trà sữa/Chè', icon: '🧋' },
]

const budgetOptions = [
  { id: 'all', label: 'Tùy duyên', icon: '🎲' },
  { id: 'rich', label: 'Đầu tháng (SSR)', icon: '👑' },
  { id: 'cheap', label: 'Cuối tháng (Bình dân)', icon: '🪙' },
]
</script>

<style scoped>
.no-scrollbar::-webkit-scrollbar {
  display: none;
}
.no-scrollbar {
  -ms-overflow-style: none;
  scrollbar-width: none;
}
</style>
